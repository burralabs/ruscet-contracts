    ...
    describe("Tests", () => {
        it("position.averagePrice, buyPrice != markPrice", async () => {
            ...
            let position = formatObj(
                await getValue(
                    vaultUtils.functions.get_position(addrToAccount(user0), toAsset(BTC), toAsset(BTC), true)
                )
            )
            ...
            let leverage = await getPositionLeverage(
                vaultStorage,
                addrToAccount(user0),
                toAsset(BTC),
                toAsset(BTC),
            ...
            let delta = formatObj(
                await getValue(
                    vaultUtils.functions.get_position_delta(addrToAccount(user0), toAsset(BTC), toAsset(BTC), true)
                )
            )
            ...
            await vault
                ...
                .addContracts(attachedContracts)
                .call()
            timestamp = await getValStr(utils.functions.get_unix_timestamp())
            position = formatObj(
                await getValue(
                    vaultUtils.functions.get_position(addrToAccount(user0), toAsset(BTC), toAsset(BTC), true)
                )
        ...
        it("short position.averagePrice, buyPrice == averagePrice", async () => {
            ...
            let position = formatObj(
                await getValue(
                    vaultUtils.functions.get_position(addrToAccount(user0), toAsset(DAI), toAsset(BTC), false)
                )
            )
            expect(position[0]).eq(toUsd(90)) // size
            expect(position[1]).eq("49910000000000000000000000000000") // collateral, 50 - 90 * 0.1%
            ...
            let delta = formatObj(
                await getValue(
                    vaultUtils.functions.get_position_delta(addrToAccount(user0), toAsset(DAI), toAsset(BTC), false)
                )
            )
            expect(delta[0]).eq(false)
            expect(delta[1]).eq("0")
        ...
        it("long position.averagePrice, buyPrice < averagePrice 2", async () => {
            ...
            let position = formatObj(
                await getValue(
                    vaultUtils.functions.get_position(addrToAccount(user0), toAsset(ETH), toAsset(ETH), true)
                )
            )
            ...
            let delta = formatObj(
                await getValue(
                    vaultUtils.functions.get_position_delta(addrToAccount(user0), toAsset(ETH), toAsset(ETH), true)
                )
            )
            expect(delta[0]).eq(false)
