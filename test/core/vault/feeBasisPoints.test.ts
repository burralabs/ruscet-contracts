...
import { getBnbConfig, getDaiConfig } from "../../utils/vault"
...
describe("Vault.getFeeBasisPoints", function () {
    ...
    it("getFeeBasisPoints", async () => {
        ...
        expect(await getValStr(vaultUtils.functions.get_fee_basis_points(toAsset(BNB), 100000, 100, 50, false))).eq(
            "150"
        )
        ...
        expect(await getValStr(vaultUtils.functions.get_fee_basis_points(toAsset(BNB), 10000, 100, 50, false))).eq(
            "116"
        )
